...
class Camera:
    ...
    # def update(self, target):
    #     # Centering target as before.
    #     target_x = -target.rect.centerx + int(self.width / 4)
    ...
    #         # If the player is on the right half + offset of the screen, move the camera to the right.
    #         if target.rect.centerx > WIN_WIDTH / 2 + EDGE_OFFSET:
    #             target_x -= target.rect.centerx - (WIN_WIDTH / 2 + EDGE_OFFSET)
    #         # If the player is on the left half - offset of the screen, move the camera to the left.
    #         elif target.rect.centerx < WIN_WIDTH / 2 - EDGE_OFFSET:
    #             target_x += (WIN_WIDTH / 2 - EDGE_OFFSET) - target.rect.centerx
    #
    #     # Clamp the camera so it doesn't show outside the level.
    #     target_x = min(0, target_x)  # left
    #     target_y = min(0, target_y)  # top
    #     target_x = max(-(self.width - WIN_WIDTH), target_x)  # right
    #     target_y = max(-(self.height - WIN_HEIGHT), target_y)  # bottom
    #
    #     # Use lerp for smoother camera movement
    #     x = self.lerp(self.camera.left, target_x, self.lerp_speed)
    #     y = self.lerp(self.camera.top, target_y, self.lerp_speed)
    #
    #     self.camera = pygame.Rect(x, y, self.width, self.height)
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.player_idle = [pygame.image.load('graphics/player/player_stand.png').convert_alpha(),
                            pygame.image.load('graphics/player/player_stand.png').convert_alpha()]  # Add idle frames
        self.player_index = 0
...
def display_score(time_left):
    # counter += 1
    # score_surf = test_font.render(f'Score: {counter}', False, (64, 64, 64))
    # score_rect = score_surf.get_rect(center=(400, 50))
    # screen.blit(score_surf, score_rect)
    # return clock_result
    # countdown_timer = 60
    # while countdown_timer >= 0 and start_countdown:
    #     time.sleep(1)
    #     countdown_timer -= 1
    # score_amt = 10 + countdown_timer
...
def get_random_questions(cursor, count=5):
    query = f"SELECT question, option1, option2, option3, correct_option FROM europe ORDER BY RAND() LIMIT {count};"
...
def display_story(screen, font):
    ...
    story_lines = [
        "Welcome to the Grand Tour of Europe!",
        "A continent steeped in history, culture, and geographical wonders.",
        "Your quest, should you decide to embark,",
        "is to journey through Europe's iconic landmarks,",
        "answering trivia to earn your title as a European Explorer.",
        "From the fjords of Norway to the beaches of Greece,",
        "your knowledge will be your compass.",
        "Are you prepared to take on this continental challenge?",
        "Press space to set sail on your European adventure!"
    ]
...
while True:
    ...
    if in_story_mode:
        ...
        # player_lives.update()
        # player_lives.current_frame += 1
        # player_lives.load_frames()
        # print(player_lives.current_frame)
        ...
        # slide_panel = SlidePanel(screen, panelSize, panelColour, textColour,
        #                          font_p, font_b, WIN_WIDTH, WIN_HEIGHT,
        #                          score, correct_answers,
        #                          time_elapsed(),
        #                          player_lives.frame_count - player_lives.current_frame)
        # slide_panel.display_value(screen, panelSize, panelColour, textColour,
        #                           font_p, font_b, WIN_WIDTH, WIN_HEIGHT,
        #                           score, correct_answers,
        #                           time_elapsed(),
        #                           player_lives.frame_count - player_lives.current_frame)
        ...
        #     score_render = test_font.render(score_line, False, (255, 255, 255))  # white color for now
        #     screen.blit(score_render, (100, start_y_position + idx * 30))  # assuming each line is 30px tall
